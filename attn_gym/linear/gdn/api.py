    ...
    *,
    ...
    Args:
        ...
        state_cache: FP32 paged state pool shaped ``[num_slots, H, V, K]``. Slots may
            ...
            must be at most 256.
        state_indices: Contiguous int32 slot indices shaped ``[B]`` for ordinary decode
            or ``[N, num_speculative_tokens + 1]`` for speculative decode. Non-positive
            indices are padding/null entries. Positive indices must be in
            ``[1, num_slots)`` and unique among concurrently updated states. These value
            constraints are caller responsibilities and are not host-validated.
        ...
        cu_seqlens: Optional packed speculative-token offsets shaped ``[N + 1]``.
            ...
            decode and requires two-dimensional ``state_indices``.
        num_accepted_tokens: Optional contiguous int32 tensor shaped ``[N]``. Entry
            ...
            ``[1, state_indices.shape[1]]``; the caller owns this value constraint.
        scale: Query scale. Defaults to ``1 / sqrt(K)``.
