        ...
        if wend > cstart:
            ...
            for chunk_idx in cutlass.range(cstart + 1, wend):
                ...
                if member == 0:
                    ...
                    tma_load_tile(
                        kq_tile, q_slice, bars.mb_kq_ready[kq_idx].smem_ptr
                    )
                    tma_load_tile(
                        kq_tile.shifted(kq_box_elems),
                        k_slice,
...
class GdnPrefillOp:
    """Compile and launch one static cudnn GDN prefill configuration."""
    ...
    def get_name(self) -> str:
        cfg = self.cfg
        return (
            f"gdn_cudnn_prefill_{cfg.io_dtype.__name__.lower()}_{cfg.state_dtype.__name__.lower()}"
