...
def validate_spec_decode_inputs(
    ...
    *,
    ...
    # The speculative launcher consumes both table strides, so no compact-layout restriction
    # is needed here.
    if (
        ...
        raise ValueError("state_indices must be int32 with shape [N, num_speculative_tokens + 1]")
    if (
...
def validate_paged_state(
    ...
    """Validate and package the shared mutable ``[slots, H, V, K]`` state contract.
