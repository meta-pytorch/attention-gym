...
"""Context parallelism for delta-rule attention: summaries, ownership plans, and state routing.
...
``dH_in = dH_out @ R + C`` the same way. The pure-PyTorch helpers below compose these maps; the per-op
``stages`` modules produce them, and the routing and collective recipe move them between ranks.
...
The short-convolution halo reuses the same slots: each fragment's slot holds the tail of its
last subsequence.
...
def _ieee_fp32_matmul() -> Iterator[None]:
    """Scope IEEE FP32 matmuls, usable as a context manager or function decorator.

    Typical summary blocks reduce over K=128: FP32 accumulation is sufficient, but TF32 operand
    rounding changes their composition. Avoid FP64, which is costly on datacenter GPUs
    with limited FP64 throughput.
    """
...
def merge_state(state: torch.Tensor, summary: torch.Tensor) -> torch.Tensor:
    ...
    The product runs in IEEE FP32 so the result does not depend on
    the caller's ``torch.get_float32_matmul_precision()``.
    """
    value_dim = state.shape[-2]
    merged = state.float() @ summary[..., value_dim:, :].float() + summary[..., :value_dim, :]
...
def compose_summaries(first: torch.Tensor, then: torch.Tensor) -> torch.Tensor:
    ...
    composed_transition = (transition.float() @ then[..., value_dim:, :].float()).to(first.dtype)
