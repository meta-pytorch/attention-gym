...
def chunk_backward_recompute(
    ...
    *,
    ...
    """Recompute chunk factors and return gradients for recurrent execution."""
    input_shapes = (q.shape, k.shape, v.shape, gate.shape, beta.shape)
    input_dtypes = (q.dtype, k.dtype, v.dtype, gate.dtype, beta.dtype)
    kernel_dtype = (
        q.dtype
    ...
    if metadata is None and (batch != 1 or tokens % _CHUNK_SIZE != 0):
        ...
        if d_output is not None:
            d_output = d_output.reshape(packed_shape)
    ...
    args = (
        ...
        False,
        autotune,
        ScheduleRequest.AUTO.value,
    )
    if initial_state is None:
        dq, dk, dv, d_cumulative, db = chunk_bwd_recompute_factors_op(*args)
        d_initial_state = None
    else:
        dq, dk, dv, d_cumulative, db, d_initial_state = (
            chunk_bwd_recompute_factors_with_state_grad_op(*args)
        )
    d_gate = _plain_gate_scan_op(
        d_cumulative,
        cu_seqlens,
    ...
    gradients = (dq, dk, dv, d_gate, db)
    gradients = tuple(
        gradient.reshape(shape).to(dtype)
        for gradient, shape, dtype in zip(
            gradients,
            input_shapes,
            input_dtypes,
            strict=True,
