...
from attn_gym.linear.kda.ops import _ReplayState
...
def paged_chunk_kda(
    ...
    *,
    ...
    Args:
        ...
        replay_state: Optional ``(q, k, v, gate, beta, count)`` persistent cache tuple.
            ...
            ``[0, 63]``, and no replay-state tensors may alias. When supplied, calls with one
            token per dense batch row use replay-backed decode; other calls use streaming
            prefill. Both paths preserve chunk boundaries across calls and support
            ``torch.compile`` and CUDA graph capture. Graph replay may change packed offsets,
            slot routing, initialization flags, and replay counts while tensor shapes remain
            fixed to the captured bucket.
    ...
    if replay_state is not None:
        replay_state = _ReplayState(*replay_state)
