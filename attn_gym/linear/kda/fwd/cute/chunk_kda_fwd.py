...
def _validate_paged_private_abi(
    ...
    if state_cache.ndim != 4 or state_cache.shape[1:] != (heads, key_dim, value_dim):
        raise ValueError(
            "the paged chunk state pool must have shape "
            f"[num_slots, {heads}, {key_dim}, {value_dim}]"
        )
    if state_cache.stride()[1:] != (key_dim * value_dim, value_dim, 1):
        raise ValueError("the paged chunk state pool must have dense [H, K, V] slots")
