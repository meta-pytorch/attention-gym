...
def chunk_delta_h_kernel_k128_wsp(
    ...
    if USE_INITIAL_STATE:
        ...
        b_h += tl.load(
            h0 + p_state + ptr_offset((o_k[:, None], o_v[None, :]), (V, 1)),
            mask=m_state,
            other=0.0,
        ).to(
            tl.float32,
    ...
    if STORE_FINAL_STATE:
        tl.store(
            ht + p_state + ptr_offset((o_k[:, None], o_v[None, :]), (V, 1)),
            b_h,
        )
...
def chunk_gated_delta_rule_fwd_h(
    ...
    *,
    ...
    if state_indices is not None:
        ...
        if initial_state.ndim != 4 or initial_state.shape[1:] != expected_state_shape[1:]:
            raise ValueError(
                "the paged state pool must have shape "
                f"[num_slots, {heads}, {key_dim}, {value_dim}], got {tuple(initial_state.shape)}"
            )
        if initial_state.dtype != torch.float32:
            raise TypeError("the paged state pool must use float32")
        if initial_state.stride()[1:] != (key_dim * value_dim, value_dim, 1):
