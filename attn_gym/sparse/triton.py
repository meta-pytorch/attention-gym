"""Triton backend for compressed sparse attention."""
...
@triton.jit
def _selected_compressed_attention_bwd_dq(
    q_ptr,
    compressed_kv_ptr,
    topk_ptr,
    output_ptr,
    ...
    TOPK: tl.constexpr,
    SCALE: tl.constexpr,
    NUM_TOPK_TILES: tl.constexpr,
    BLOCK_K: tl.constexpr,
    BLOCK_D: tl.constexpr,
):
    """Gather selected values and use tensor-core tiles to accumulate query gradients."""
    query_position = tl.program_id(0)
    batch_head = tl.program_id(1)
    head = batch_head % H
    batch = batch_head // H
    offsets_d = tl.arange(0, BLOCK_D)
    offsets_k_base = tl.arange(0, BLOCK_K)
    offsets_dot = tl.arange(0, 16)
    dimension_mask = offsets_d < D

    query_offset = (
        batch * stride_qb + head * stride_qh + query_position * stride_qs + offsets_d * stride_qd
    )
    query = tl.load(q_ptr + query_offset, mask=dimension_mask, other=0.0)
    output = tl.load(output_ptr + query_offset, mask=dimension_mask, other=0.0)
    grad_output = tl.load(grad_output_ptr + query_offset, mask=dimension_mask, other=0.0)
    lse = tl.load(lse_ptr + batch * stride_leb + head * stride_leh + query_position * stride_les)
    delta = tl.sum(grad_output * output, axis=0)
    grad_query = tl.load(grad_q_ptr + query_offset, mask=dimension_mask, other=0.0).to(tl.float32)
    completed_blocks = (query_position + 1) // COMPRESSION_RATE
    dot_inputs = tl.where(
        offsets_dot[None, :] == 0,
        query[:, None],
        tl.where(offsets_dot[None, :] == 1, grad_output[:, None], 0.0),
    )
    dot_rows = tl.arange(0, 16)

    for topk_tile in tl.static_range(0, NUM_TOPK_TILES):
        offsets_k = topk_tile * BLOCK_K + offsets_k_base
        selected_blocks = tl.load(
            topk_ptr + batch * stride_tb + query_position * stride_ts + offsets_k * stride_tk,
            mask=offsets_k < TOPK,
            other=0,
        ...
        selected_values = tl.load(
            compressed_kv_ptr + compressed_offsets,
            mask=valid[:, None] & dimension_mask[None, :],
            other=0.0,
        )
        score_and_grad_probability = tl.dot(
            selected_values,
            dot_inputs,
        ...
        scores = (
            tl.sum(
                score_and_grad_probability * (offsets_dot[None, :] == 0),
                axis=1,
            )
            * SCALE
        )
        grad_probabilities = tl.sum(
            score_and_grad_probability * (offsets_dot[None, :] == 1),
            axis=1,
        )
        probabilities = tl.where(valid, tl.exp(scores - lse), 0.0)
        grad_scores = probabilities * (grad_probabilities - delta)
        grad_score_tile = tl.where(
            dot_rows[:, None] == 0,
            grad_scores[None, :],
            0.0,
        )
        grad_query_tile = tl.dot(
            grad_score_tile.to(selected_values.dtype),
            selected_values,
            input_precision="tf32x3",
        )
        grad_query += tl.sum(grad_query_tile * (dot_rows[:, None] == 0), axis=0) * SCALE

    tl.store(grad_q_ptr + query_offset, grad_query, mask=dimension_mask)


@triton.jit
def _selected_compressed_attention_bwd_dkv(
    q_ptr,
    compressed_kv_ptr,
    ...
    """Group selected queries by compressed block and reduce value gradients with tl.dot."""
    ...
    for entry_tile in tl.range(entry_start, entry_end, BLOCK_M):
        ...
        grad_probability_tile = tl.dot(
            grad_output,
            dot_value,
            input_precision="tf32x3",
        )
        grad_probabilities = tl.sum(
            grad_probability_tile * (dot_rows[None, :] == 0),
            axis=1,
        )
        delta = tl.sum(grad_output * output, axis=1)
        probabilities = tl.where(query_mask, tl.exp(scores - lse), 0.0)
        grad_scores = probabilities * (grad_probabilities - delta)
        combined_weights = tl.cat(
            probabilities,
            grad_scores * SCALE,
            dim=0,
        )
        combined_values = tl.cat(
            grad_output,
            query,
            dim=0,
        )
...
def _build_selected_query_index(
    topk_blocks: torch.Tensor,
    num_blocks: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Invert query-major top-k indices into block-major query lists."""
    batch, sequence_length, topk = topk_blocks.shape
    if topk == 0:
        selected_queries = torch.empty(
            batch,
            0,
    ...
    flat_blocks = topk_blocks.flatten(1)
    sorted_blocks, sorted_entries = torch.sort(flat_blocks, dim=-1)
    selected_queries = torch.div(sorted_entries, topk, rounding_mode="floor").to(torch.int32)
    block_ids = torch.arange(
        num_blocks + 1,
        device=topk_blocks.device,
        dtype=sorted_blocks.dtype,
    )
    block_ids = block_ids.unsqueeze(0).expand(batch, -1).contiguous()
    block_offsets = torch.searchsorted(sorted_blocks, block_ids).to(torch.int32)
    assert selected_queries.shape == (batch, sequence_length * topk)
...
def _launch_selected_attention_backward(
    ...
    if topk:
        ...
        compressed_block_k = 32 if topk >= 32 else 16
        _selected_compressed_attention_bwd_dq[(sequence_length, batch * heads)](
            query,
            compressed_kv,
            topk_blocks,
            output,
            ...
            *lse.stride(),
            ...
            TOPK=topk,
            SCALE=scale,
            NUM_TOPK_TILES=triton.cdiv(topk, compressed_block_k),
            BLOCK_K=compressed_block_k,
            BLOCK_D=block_d,
            num_warps=4,
        )
        compressed_block_m = 32 if head_dim <= 128 else 16
        _selected_compressed_attention_bwd_dkv[(num_blocks, batch * heads)](
            query,
            compressed_kv,
            selected_queries,
            block_offsets,
        ...
        grad_compressed_kv = torch.zeros(
            batch,
            heads,
            num_blocks,
            head_dim,
...
class _SelectedAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        selected_queries, block_offsets = _build_selected_query_index(
            topk_blocks,
...
def _prepare_attention_inputs(
    ...
    *,
    ...
    _batch, heads, sequence_length, head_dim = Q.shape
