...
class IndexTopK:
    """Fuse QI RoPE, shared-key index matmuls, weighting, causal top-k, and padding."""
    ...
    def __init__(
        ...
        assert gather_length >= topk and gather_length % 128 == 0
    ...
    def kernel(
        ...
        best_values = cute.make_rmem_tensor((self.topk,), Float32)
        best_indices = cute.make_rmem_tensor((self.topk,), Int32)
        if tid == 0:
        ...
        # Stream over blocks so shared-memory use and generated code are independent of context
        # length. Four warps process index heads in groups; thread 0 reduces the published
        # contributions and maintains the row's small top-k set in registers.
        for n in cutlass.range(self.num_blocks, unroll=1):
            if tid == 0:
...
class MergeSinkInverseRope:
    ...
    def __call__(
        ...
        self.kernel(
            local_out, local_lse, compressed_out, compressed_lse, sink, cos, sin, out
        ).launch(
...
def compile_prefix(
    ...
    return cute.compile(
        ...
        _fake(dtype, (batch, prefix, tile_heads, dim)),
