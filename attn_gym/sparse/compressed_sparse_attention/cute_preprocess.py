...
class _CompressRmsNormRopeSm100:
    ...
    def kernel(
        ...
        warp_sums = allocator.allocate_array(Float32, _WARPS)

        values_per_thread = cute.ceil_div(dimension, _THREADS)
        for item in cutlass.range_constexpr(values_per_thread):
            ...
            if d < dimension:
                ...
                for offset in cutlass.range_constexpr(rate):
                    ...
                    denominator += cute.math.exp(logit_a - maximum, fastmath=False)
                    if block > 0:
                        source_b = (block - 1) * rate + offset
                        logit_b = Float32(
                            out_dtype(
                        ...
                        denominator += cute.math.exp(logit_b - maximum, fastmath=False)
...
def parallel_preprocess_stream(KV: torch.Tensor) -> torch.cuda.Stream | None:
    if KV.dtype != torch.bfloat16 or KV.shape[0] != 1 or KV.shape[2] < 1024:
        return None
    device = KV.device
