...
class PackDsaKvIndices:
    def __init__(self, dsa_dtype, batch, sequence, dim, blocks, topk, window, width):
        self.dtype = dsa_dtype
        self.batch = batch
        self.sequence = sequence
        self.dim = dim
    ...
    @cute.jit
    def __call__(self, compressed, local, gather, kv, indices, sink, sink_fp32, stream: cuda.CUstream):
        ...
        self.index_kernel(gather, indices).launch(
    ...
    @cute.kernel
    def index_kernel(self, gather, indices):
        tid, _, _ = cute.arch.thread_idx(); bid, _, _ = cute.arch.block_idx()
        linear = bid * 256 + tid
        ...
        if linear < total:
            ...
            elif slot < self.topk + self.window:
                key = pos - self.window + 1 + (slot - self.topk)
                if key >= 0:
...
@lru_cache
def compile_pack_dsa_kv_indices(input_dtype, dsa_dtype, batch, sequence, dim, blocks, topk, window, width, heads):
    gather_width = max(1, math.ceil(topk / 128) * 128)
    return cute.compile(
        PackDsaKvIndices(dsa_dtype, batch, sequence, dim, blocks, topk, window, width),
