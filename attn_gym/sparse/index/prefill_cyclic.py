"""Experimental two-level cyclic-buffer DSA/CSA prefill indexer for SM100.

This is an isolated copy of :mod:`prefill`; the production kernel remains
untouched.  Tensor-core producers feed reduced M128 score tiles into a raw
mailbox.  One ingress warp per query releases that mailbox promptly, compacts
survivors against a monotonic global cutoff, and publishes variable-length
pages into a second cyclic ring.  Four drain warps batch those pages into M512
runs and maintain one exact, query-wide Top-128 while later GEMMs continue.
There are no global partial lists, atomics, compile-cache scaffolds, or fallback
implementations.
"""
...
_QUERIES_PER_CTA = 2
# Four reducer warps consume each M128xN64 TMEM tile.  A distinct ingress warp
# compacts the reduced stream, then four drain warps jointly update one Top-128.
_REDUCER_WARPS = 4
_REDUCER_THREADS = _REDUCER_WARPS * 32
_INGRESS_WARPS = 1
_INGRESS_THREADS = _INGRESS_WARPS * 32
_DRAIN_WARPS = 4
_DRAIN_THREADS = _DRAIN_WARPS * 32
_TOTAL_REDUCER_WARPS = _QUERIES_PER_CTA * _REDUCER_WARPS
_TOTAL_INGRESS_WARPS = _QUERIES_PER_CTA * _INGRESS_WARPS
_TOTAL_DRAIN_WARPS = _QUERIES_PER_CTA * _DRAIN_WARPS
# The names below belong to the copied baseline selector helpers.  They remain
# defined so the copy stays easy to diff, but the cyclic kernel does not call
# those helpers.
_SELECTION_WARPS = 4
...
_DRAIN_WARP_BASE = _TOTAL_REDUCER_WARPS
_INGRESS_WARP_BASE = _DRAIN_WARP_BASE + _TOTAL_DRAIN_WARPS
_MMA_WARP = _INGRESS_WARP_BASE + _TOTAL_INGRESS_WARPS
_LOAD_WARP = _MMA_WARP + 1
_THREADS = (_LOAD_WARP + 1) * 32
_ACC_TMEM_STAGES = _QUERIES_PER_CTA * _ACC_STAGES
_DRAIN_SORT_SPAN = 4 * _TILE_CANDIDATES
_DRAIN_SCRATCH_KEYS = _TILE_CANDIDATES
_DRAIN_WORKSPACE_KEYS = _QUERIES_PER_CTA * (
    _DRAIN_SORT_SPAN + _DRAIN_SCRATCH_KEYS
)
_SURVIVOR_RING_KEYS = (
    _QUERIES_PER_CTA * _SURVIVOR_STAGES * _TILE_CANDIDATES
)
_SURVIVOR_RING_COUNTS = _QUERIES_PER_CTA * _SURVIVOR_STAGES
_MIN_SORT_SPAN = 512
_ALIGNMENT = 16
_MAX_SEQUENCE = 1 << 20
_INVALID_KEY = -(1 << 63)
...
class _SharedStorage:
    ...
    survivor0_barriers: cute.struct.MemRange[Int64, _SURVIVOR_STAGES * 2]
    survivor1_barriers: cute.struct.MemRange[Int64, _SURVIVOR_STAGES * 2]
    tmem_holding: Int32
...
@dsl_user_op
def _volatile_load_i32(pointer, *, loc=None, ip=None) -> Int32:
    llvm_pointer = pointer.to_llvm_ptr(loc=loc, ip=ip)
    return Int32(
        llvm.load(
            T.i32(),
            llvm_pointer,
...
@cute.jit
def _sort_warp_values_desc(values: cute.Tensor, lane: Int32):
    """Sort four striped Int64 values per lane into one descending M128 run."""
        ...
        for reverse_stage in cutlass.range_constexpr(level + 1):
            ...
            if cutlass.const_expr(distance < _WARP_SIZE):
                for item in cutlass.range_constexpr(4):
                    logical_index = lane + Int32(item * _WARP_SIZE)
                    other = Int64(cute.arch.shuffle_sync_bfly(values[item], distance))
                    lower = (logical_index & Int32(distance)) == Int32(0)
                    descending = (logical_index & Int32(network_size)) == Int32(0)
...
@cute.jit
def _merge_warp_top128_desc(
    long_term: cute.Tensor,
    incoming: cute.Tensor,
    lane: Int32,
):
    """Keep the descending Top-128 of two register-resident M128 runs."""
    # Compare the descending long-term run with the reversed incoming run.
    # The maxima form a bitonic M128 sequence containing exactly the upper
    # half of the M256 union.
    for item in cutlass.range_constexpr(4):
        reversed_value = Int64(
            cute.arch.shuffle_sync_bfly(incoming[3 - item], _WARP_SIZE - 1)
        )
        current = Int64(long_term[item])
        long_term[item] = current if current > reversed_value else reversed_value

    # A bitonic merge, rather than a second full sort, orders that upper half.
    for reverse_stage in cutlass.range_constexpr(7):
        distance = 1 << (6 - reverse_stage)
        if cutlass.const_expr(distance < _WARP_SIZE):
            for item in cutlass.range_constexpr(4):
                logical_index = lane + Int32(item * _WARP_SIZE)
                other = Int64(
                    cute.arch.shuffle_sync_bfly(long_term[item], distance)
                )
                lower = (logical_index & Int32(distance)) == Int32(0)
                long_term[item] = _bitonic_lane_value(
                    long_term[item],
                    other,
                    lower,
            ...
            for item in cutlass.range_constexpr(4):
                partner_item = item ^ register_distance
                if cutlass.const_expr(partner_item > item):
                    value = Int64(long_term[item])
                    other = Int64(long_term[partner_item])
                    larger = value if value > other else other  # noqa: FURB136
                    smaller = other if value > other else value  # noqa: FURB136
                    long_term[item] = larger
                    long_term[partner_item] = smaller


@cute.jit
def _bitonic_merge_warp_desc(values: cute.Tensor, lane: Int32):
    """Order one register-resident bitonic M128 sequence descending."""
    for reverse_stage in cutlass.range_constexpr(7):
        distance = 1 << (6 - reverse_stage)
        if cutlass.const_expr(distance < _WARP_SIZE):
            for item in cutlass.range_constexpr(4):
...
def _drain_tree_512_128(
    ...
    query_slot: cutlass.Constexpr,
    selection_barrier,
    final_drain: cutlass.Constexpr,
):
    """Retain Top-128 from M512 with a 4→2→1 warp merge tree."""
    values = cute.make_rmem_tensor(cute.make_layout((4,)), Int64)
    run_base = query_base + warp_idx * Int32(_TILE_CANDIDATES)
    for item in cutlass.range_constexpr(4):
        slot = lane + Int32(item * _WARP_SIZE)
        values[item] = Int64(keys[run_base + slot])
    _sort_warp_values_desc(values, lane)

    # Odd warps publish their sorted runs; even warps retain theirs in
    # registers and compare-split each adjacent M128 pair concurrently.
    if (warp_idx & Int32(1)) == Int32(1):
        for item in cutlass.range_constexpr(4):
            slot = lane + Int32(item * _WARP_SIZE)
            keys[run_base + slot] = values[item]
    cute.arch.fence_view_async_shared()
    selection_barrier.arrive_and_wait()

    scratch_base = Int32(_QUERIES_PER_CTA * _DRAIN_SORT_SPAN) + Int32(
        query_slot * _DRAIN_SCRATCH_KEYS
    )
    if (warp_idx & Int32(1)) == Int32(0):
        other_base = run_base + Int32(_TILE_CANDIDATES)
        for item in cutlass.range_constexpr(4):
            slot = lane + Int32(item * _WARP_SIZE)
            current = Int64(values[item])
            other = Int64(
                keys[
                    other_base + Int32(_TILE_CANDIDATES - 1) - slot
                ]
            )
            values[item] = current if current > other else other
        _bitonic_merge_warp_desc(values, lane)
        if warp_idx == Int32(2):
            for item in cutlass.range_constexpr(4):
                slot = lane + Int32(item * _WARP_SIZE)
                keys[scratch_base + slot] = values[item]
    cute.arch.fence_view_async_shared()
    selection_barrier.arrive_and_wait()
    ...
    if warp_idx == Int32(0):
        for item in cutlass.range_constexpr(4):
            ...
            other = Int64(
                keys[
                    scratch_base + Int32(_TILE_CANDIDATES - 1) - slot
                ]
            )
            values[item] = current if current > other else other
        _bitonic_merge_warp_desc(values, lane)
        for item in cutlass.range_constexpr(4):
            slot = lane + Int32(item * _WARP_SIZE)
            keys[query_base + slot] = values[item]

    # The next accumulation cycle may place a survivor into any tail slot.
    # Clear every loser while warp 0 publishes the disjoint retained prefix.
    for item in cutlass.range_constexpr(3):
        keys[
            query_base
            + Int32(_TILE_CANDIDATES)
            + warp_idx * Int32(_WARP_SIZE)
            + lane
            + Int32(item * _DRAIN_THREADS)
        ] = Int64(_INVALID_KEY)
    cute.arch.fence_view_async_shared()
    if cutlass.const_expr(not final_drain):
        selection_barrier.arrive_and_wait()
...
def _run_query_ingress(
    ...
    """Compact raw M128 score tiles into a variable-length survivor ring."""
    mailbox_query_base = Int32(
        query_slot * _MAILBOX_STAGES * _TILE_CANDIDATES
    )
    survivor_query_base = Int32(
        query_slot * _SURVIVOR_STAGES * _TILE_CANDIDATES
    )
    count_query_base = Int32(query_slot * _SURVIVOR_STAGES)
    lane_in_group = lane & Int32(3)
    candidate_group = lane >> Int32(2)
    ordinals = cute.make_rmem_tensor(cute.make_layout((4,)), Int32)

    if lane == Int32(0):
        cutoff_ordinals[query_slot] = Int32(_INVALID_ORDINAL)
    cute.arch.fence_view_async_shared()
    cute.arch.sync_warp()
    ...
    for candidate_tile in cutlass.range(candidate_tiles, unroll=0):
        ...
        for producer_warp in cutlass.range_constexpr(_REDUCER_WARPS):
            ordinals[producer_warp] = Int32(
                mailbox_ordinals[
                    mailbox_stage_base
                    + Int32(producer_warp * _WARP_SIZE)
                    + lane
                ]
            )
        # The entire raw tile is now in registers, so reducers can immediately
        # recycle this stage while ingress waits for survivor-ring capacity.
        cute.arch.fence_view_async_shared()
        ...
        if lane == Int32(0):
            cutoff_ptr = (
                cutoff_ordinals.iterator
                + cutoff_ordinals.layout(Int32(query_slot))
            )
            cutoff_ordinal = _volatile_load_i32(cutoff_ptr)
        cutoff_ordinal = Int32(cute.arch.shuffle_sync(cutoff_ordinal, 0))

        survivor_empty = survivor_producer.acquire_and_advance()
        survivor_stage_base = (
            survivor_query_base
            + survivor_empty.index * Int32(_TILE_CANDIDATES)
        )
        local_count = Int32(0)
        for producer_warp in cutlass.range_constexpr(_REDUCER_WARPS):
            candidate_ordinal = Int32(ordinals[producer_warp])
            accept = (
                candidate_ordinal != Int32(_INVALID_ORDINAL)
                and candidate_ordinal >= cutoff_ordinal
            )
            if accept:
                local_count = local_count + Int32(1)

        inclusive_count = local_count
        for shuffle_stage in cutlass.range_constexpr(5):
            distance = 1 << shuffle_stage
            other = Int32(
            ...
            if lane >= Int32(distance):
                inclusive_count = inclusive_count + other
        accepted_base = Int32(
            cute.arch.shuffle_sync(inclusive_count, _WARP_SIZE - 1)
        )
        lane_base = inclusive_count - local_count
        lane_rank = Int32(0)
        for producer_warp in cutlass.range_constexpr(_REDUCER_WARPS):
            candidate_ordinal = Int32(ordinals[producer_warp])
            candidate_offset = (
            ...
            candidate_index = (
                candidate_tile * Int32(_TILE_CANDIDATES) + candidate_offset
            )
            accept = (
                candidate_ordinal != Int32(_INVALID_ORDINAL)
                and candidate_ordinal >= cutoff_ordinal
            ...
            if accept:
                survivor_keys[
                    survivor_stage_base + lane_base + lane_rank
                ] = _pack_key(candidate_ordinal, candidate_index)
                lane_rank = lane_rank + Int32(1)

        # A complete first M128 tile is already a certified Top-128 seed.  Its
        # minimum ordinal is therefore a safe early filter while the drain
        # warps are still sorting that page.  Publish before commit so a later
        # (higher) drain cutoff cannot be overwritten by this seed value.
        if candidate_tile == Int32(0) and accepted_base == Int32(_TILE_CANDIDATES):
            seed_min = Int32(ordinals[0])
            for item in cutlass.range_constexpr(3):
                other = Int32(ordinals[item + 1])
            ...
            for shuffle_stage in cutlass.range_constexpr(5):
                ...
                seed_min = other if other < seed_min else seed_min  # noqa: FURB136
            if lane == Int32(0):
                cutoff_ordinals[query_slot] = seed_min

        if lane == Int32(0):
            survivor_counts[
                count_query_base + survivor_empty.index
            ] = accepted_base
        cute.arch.fence_view_async_shared()
        survivor_empty.commit()

    survivor_producer.tail()


@cute.jit
def _run_query_survivor_drain(
    survivor_keys,
    survivor_counts,
    survivor_consumer,
    drain_keys,
    ...
    """Batch cyclic survivor pages and maintain one exact Top-128 per query.

    Four warps append compact pages without atomics.  When the M512 workspace
    would overflow, they cooperatively sort four M128 runs and retain the best
    128.  Later survivors refill the cleared 384-key tail.
    """
    barrier_id = 3
    if cutlass.const_expr(query_slot == 1):
        barrier_id = 4
    drain_barrier = pipeline.NamedBarrier(
        barrier_id=barrier_id,
        num_threads=_DRAIN_THREADS,
    )
    survivor_query_base = Int32(
        query_slot * _SURVIVOR_STAGES * _TILE_CANDIDATES
    )
    count_query_base = Int32(query_slot * _SURVIVOR_STAGES)
    drain_query_base = Int32(query_slot * _DRAIN_SORT_SPAN)
    local_thread = warp_idx * Int32(_WARP_SIZE) + lane

    for item in cutlass.range_constexpr(4):
        drain_keys[
            drain_query_base + local_thread + Int32(item * _DRAIN_THREADS)
        ] = Int64(_INVALID_KEY)
    cute.arch.fence_view_async_shared()
    drain_barrier.arrive_and_wait()

    occupied = Int32(0)
    for _ in cutlass.range(candidate_tiles, unroll=0):
        survivor_full = survivor_consumer.wait_and_advance()
        survivor_stage_base = (
            survivor_query_base
            + survivor_full.index * Int32(_TILE_CANDIDATES)
        )
        page_count = Int32(0)
        if lane == Int32(0):
            page_count = Int32(
                survivor_counts[count_query_base + survivor_full.index]
            )
        page_count = Int32(cute.arch.shuffle_sync(page_count, 0))

        if occupied + page_count > Int32(_DRAIN_SORT_SPAN):
            cute.arch.fence_view_async_shared()
            drain_barrier.arrive_and_wait()
            _drain_tree_512_128(
                drain_keys,
            ...
            if local_thread == Int32(0):
                cutoff_ordinals[query_slot] = Int32(
                    Int64(
                        drain_keys[
                            drain_query_base + Int32(_TILE_CANDIDATES - 1)
                        ]
                    )
                    >> Int64(32)
                )
            cute.arch.fence_view_async_shared()
            occupied = Int32(_TILE_CANDIDATES)

        page_slot = local_thread
        if page_slot < page_count:
            drain_keys[drain_query_base + occupied + page_slot] = Int64(
                survivor_keys[survivor_stage_base + page_slot]
            )
        cute.arch.fence_view_async_shared()
        survivor_full.release()
        occupied = occupied + page_count
...
def _prefill_index_kernel(
    ...
    drain_keys = smem.allocate_tensor(
        Int64,
        cute.make_layout((_DRAIN_WORKSPACE_KEYS,)),
        byte_alignment=128,
    )
    survivor_keys = smem.allocate_tensor(
        Int64,
        cute.make_layout((_SURVIVOR_RING_KEYS,)),
        byte_alignment=128,
    )
    survivor_counts = smem.allocate_tensor(
        Int32,
        cute.make_layout((_SURVIVOR_RING_COUNTS,)),
        byte_alignment=16,
    )
    cutoff_ordinals = smem.allocate_tensor(
        Int32,
    ...
    survivor0_producer, survivor0_consumer = pipeline.PipelineAsync.create(
        num_stages=_SURVIVOR_STAGES,
        producer_group=pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            _INGRESS_THREADS,
        ),
        ...
        barrier_storage=storage.survivor0_barriers.data_ptr(),
        defer_sync=True,
    ).make_participants()
    survivor1_producer, survivor1_consumer = pipeline.PipelineAsync.create(
        num_stages=_SURVIVOR_STAGES,
        producer_group=pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            _INGRESS_THREADS,
        ...
        barrier_storage=storage.survivor1_barriers.data_ptr(),
        defer_sync=True,
    ).make_participants()
    ...
    if warp_idx < Int32(_REDUCER_WARPS):
    ...
    elif warp_idx < Int32(_TOTAL_REDUCER_WARPS):
        _run_query_reducer_mailbox(
            accumulator,
            acc1_consumer,
            mailbox_ordinals,
            mailbox1_producer,
            mW_bth,
            tidx - Int32(_REDUCER_THREADS),
            warp_idx - Int32(_REDUCER_WARPS),
            lane,
            batch,
            query1_load,
            query1_active,
    ...
    elif warp_idx == Int32(_INGRESS_WARP_BASE):
        _run_query_ingress(
            mailbox_ordinals,
            mailbox0_consumer,
            survivor_keys,
    ...
    elif warp_idx == Int32(_INGRESS_WARP_BASE + 1):
        _run_query_ingress(
            mailbox_ordinals,
            mailbox1_consumer,
            survivor_keys,
    ...
    elif warp_idx < Int32(_DRAIN_WARP_BASE + _DRAIN_WARPS):
        _run_query_survivor_drain(
            survivor_keys,
            survivor_counts,
            survivor0_consumer,
            drain_keys,
            cutoff_ordinals,
            warp_idx - Int32(_DRAIN_WARP_BASE),
            lane,
            0,
            candidate_tiles,
        )
    elif warp_idx < Int32(_DRAIN_WARP_BASE + _TOTAL_DRAIN_WARPS):
        _run_query_survivor_drain(
            survivor_keys,
            survivor_counts,
            survivor1_consumer,
            drain_keys,
            cutoff_ordinals,
            warp_idx - Int32(_DRAIN_WARP_BASE + _DRAIN_WARPS),
            lane,
            1,
            candidate_tiles,
        )
    ...
    if warp_idx < Int32(_DRAIN_WARP_BASE + _DRAIN_WARPS) and warp_idx >= Int32(
        _DRAIN_WARP_BASE
    ):
        rank = (warp_idx - Int32(_DRAIN_WARP_BASE)) * Int32(_WARP_SIZE) + lane
        key = Int64(drain_keys[rank])
        mOut_btk[batch, query0, rank] = ~Int32(key & Int64(0xFFFFFFFF))
    elif (
        warp_idx >= Int32(_DRAIN_WARP_BASE + _DRAIN_WARPS)
        and warp_idx < Int32(_DRAIN_WARP_BASE + _TOTAL_DRAIN_WARPS)
        and query1_active
    ):
        rank = (
            warp_idx - Int32(_DRAIN_WARP_BASE + _DRAIN_WARPS)
        ) * Int32(_WARP_SIZE) + lane
        key = Int64(
            drain_keys[Int32(_DRAIN_SORT_SPAN) + rank]
        )
        mOut_btk[batch, query1, rank] = ~Int32(key & Int64(0xFFFFFFFF))
    tmem.free(tmem_ptr)
...
def _validate(q: torch.Tensor, k: torch.Tensor, weights: torch.Tensor, topk: int) -> None:
    ...
    if topk != _TILE_CANDIDATES:
        raise ValueError(
            "the cyclic experimental kernel currently requires topk == "
            f"{_TILE_CANDIDATES}, got {topk}"
        )
    if topk > queries:
        raise ValueError(f"topk must not exceed sequence length {queries}, got {topk}")
...
def index(
    ...
    """Return the shared-key Top-128 for every full-sequence query.
