...
def generate_inputs(
    ...
    make_q = partial(torch.rand, q_shape, device=device, dtype=dtype, requires_grad=requires_grad)
    make_kv = partial(
        torch.rand, kv_shape, device=device, dtype=dtype, requires_grad=requires_grad
    )

    if nested_tensors:
        query = make_q().view(1, q_sequence_length * batch_size, q_heads, head_dim).transpose(1, 2)
        key = (
            make_kv().view(1, batch_size * kv_sequence_length, kv_heads, head_dim).transpose(1, 2)
        )
        value = (
            make_kv().view(1, batch_size * kv_sequence_length, kv_heads, head_dim).transpose(1, 2)
        )
    else:
        query = make_q().view(batch_size, q_sequence_length, q_heads, head_dim).transpose(1, 2)
        key = make_kv().view(batch_size, kv_sequence_length, kv_heads, head_dim).transpose(1, 2)
        value = make_kv().view(batch_size, kv_sequence_length, kv_heads, head_dim).transpose(1, 2)
...
def calculate_bandwidth(
    ...
    sparsity = (results.sparsity if results.sparsity is not None else 0.0) if M == 1 else 0.0
    ...
    if type == "fwd":
        ...
        total_size = (query_size + kv_size * (1 - sparsity) + output_size) / 1e9  # In GB
        ...
        k_reads_dq = 1 * B * Hkv * N * D * bytes_per_element * (1 - sparsity)  # K (sparse)
        v_reads_dq = 1 * B * Hkv * N * D * bytes_per_element * (1 - sparsity)  # V (sparse)
        ...
        k_reads_dk = 1 * B * Hkv * N * D * bytes_per_element * (1 - sparsity)  # K (sparse)
        v_reads_dk = 1 * B * Hkv * N * D * bytes_per_element * (1 - sparsity)  # V (sparse)
...
def add_metrics_to_result(
    ...
    # Forward metrics
    if results.fwd_time is None or math.isnan(results.fwd_time) or results.fwd_time <= 0:
    ...
    # Backward metrics
    if results.bwd_time is None or math.isnan(results.bwd_time) or results.bwd_time <= 0:
...
def print_results(
    ...
    if show_speedups:
        for backend in results[0].config.backends:
            fwd_speedups = [
                calculate_speedup(r.results["flex_attn"], r.results[backend], type="fwd")
                for r in results
            ]
        ...
        if results[0].config.calculate_bwd_time:
            for backend in results[0].config.backends:
                bwd_speedups = [
                    calculate_speedup(r.results["flex_attn"], r.results[backend], type="bwd")
                    for r in results
                ]
        ...
        for backend in results[0].config.backends:
            if (
                ...
                if results[0].config.calculate_bwd_time:
                    ...
                    print(
                        tabulate(average_data, headers="keys", tablefmt="github", floatfmt=".3f")
                    )
...
def get_kernel_options(
    ...
    return (
        kernel_opt_decoding_dict[attn_type] if is_decoding else kernel_opt_training_dict[attn_type]
    )
    ...
    **kwargs,
    ...
    if backend == "fav2":
        try:
            ...
            print("Flash attention 2 is not installed. Please install it to run fav2 backend. ")
            raise
    elif backend == "fav3":
        try:
...
def generate_FD_callable(
    ...
    FD_dict = {
        ...
        "rel": None,
        "head_bias": None,
        "alibi": partial(flash_attn_with_kvcache_renamed, cache_seqlens=N // 2, **FA_kwargs),
        "sliding_window": partial(
...
def generate_eager_sdpa(
    ...
    **kwargs,
    ...
    if attn_type == "sliding_window" or attn_type == "prefix_lm":
        ...
        attn_mask = generate_attn_mask_linear_score_mod((1, 1, M, N), block_mask, score_mod, dtype)
