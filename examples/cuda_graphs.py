...
def hello_world_training_loop(use_monitor: bool = False) -> Tensor:
    ...
    def forward_backward() -> tuple[Tensor, tuple[Tensor, ...]]:
        ...
        with mark_kernels("attention"):  # (5)!
            output = model(inputs, static_cu_seqlens, max_seqlen)
    ...
    trace_profiler = training_loop_profiler(use_monitor)
    graph, (loss, graph_grads) = capture_graph(
        forward_backward,
        enable_annotations=use_monitor,
    )
    graph.replay()  # (7)!
...
# --8<-- [start:monitor-training-loop]
def training_loop_profiler(use_monitor: bool):
    if use_monitor:
        return profiler(
            TRACE_PATH / "docs/assets/traces/hello_world_training_loop_monitor",
            record_shapes=False,
            backend="cupti_monitor",
        )
    return profiler(
        TRACE_PATH / "docs/assets/traces/hello_world_training_loop",
        record_shapes=False,
    )


def hello_world_training_loop_monitor() -> Tensor:
    return hello_world_training_loop(use_monitor=True)


# --8<-- [end:monitor-training-loop]
...
class Example(str, Enum):
    ...
    HELLO_WORLD_TRAINING_MONITOR = "hwtm"
...
def main(example: Example) -> None:
    match example:
        ...
        case Example.HELLO_WORLD_TRAINING_MONITOR:
