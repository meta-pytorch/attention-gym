...
End-to-end examples grouped by the same high-level concepts as the library. Source files live
at most one directory below `examples/`:
...
[`examples/flex_attention/flex_attn.ipynb`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/flex_attn.ipynb) — Comprehensive Jupyter notebook covering the full FlexAttention API: basic usage, score_mod vs mask_mod, causal masking, sliding window, prefix LM, document masking, NATTEN (with tiled layout), ALiBi, tanh soft-capping, nested jagged tensors, and Flamingo cross-attention. Each section includes performance benchmarks against `F.scaled_dot_product_attention`.
...
[`examples/flex_attention/debug_score_mod.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/debug_score_mod.py) — Interactive [marimo](https://marimo.io) notebook that walks through debugging a broken `score_mod` step-by-step. Demonstrates the `_FLEX_ATTENTION_DISABLE_COMPILE_DEBUG` flag and how to use `get_unwrapped` to inspect tensor values inside vmap. See [Concepts - Debugging](concepts.md#debugging-score_mods) for more details.
...
[`examples/flex_attention/benchmark.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/benchmark.py) — Compare FlexAttention against causal FA2 and `F.sdpa` with a dense mask across multiple attention patterns.

```bash
# Run all benchmarks
python examples/flex_attention/benchmark.py

# Run specific patterns
python examples/flex_attention/benchmark.py --examples causal sliding_window document
```
...
[`examples/flex_attention/flex_flash_attention.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/flex_flash_attention.py) — Compare the Flash (CuTeDSL-based) and Triton backends for FlexAttention.

```bash
python examples/flex_attention/flex_flash_attention.py                    # Run all
python examples/flex_attention/flex_flash_attention.py --mode benchmark   # Just performance
python examples/flex_attention/flex_flash_attention.py --mode compare     # Just numerical accuracy
```
...
[`examples/flex_attention/mla.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/mla.py) — DeepSeek-V2 style MLA with weight absorption, showing how FlexAttention can express the absorbed RoPE score modification.

```bash
python examples/flex_attention/mla.py --mode acc     # Accuracy test (vanilla vs absorbed vs flex)
python examples/flex_attention/mla.py --mode perf    # Performance comparison
```
...
[`examples/flex_attention/flex_determinism.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/flex_determinism.py) — Test bitwise determinism of FlexAttention across multiple compilation settings and tensor shapes.
...
[`examples/flex_attention/ring_attention.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/ring_attention.py) — Build a custom distributed ring-attention op by directly invoking FlexAttention's forward and backward primitives.
...
```bash
torchrun --standalone --nproc_per_node=4 examples/flex_attention/ring_attention.py --seq-len 131072
```
...
loss/backward, and capture recipes are shown in `examples/linear/delta_rule_training.py` and imported
...
```bash
torchrun --standalone --nproc_per_node=2 examples/linear/delta_rule_context_parallel.py --batch-size 4 --tokens 1024

torchrun --standalone --nproc_per_node=2 examples/linear/delta_rule_context_parallel.py --partition zigzag

torchrun --standalone --nproc_per_node=2 examples/linear/delta_rule_context_parallel.py --compute-dtype=float16

torchrun --standalone --nproc_per_node=2 examples/linear/delta_rule_context_parallel.py --cuda-graph

torchrun --standalone --nproc_per_node=2 examples/linear/delta_rule_context_parallel.py --profile
```
...
[`examples/flex_attention/flex_autotune_replay.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/flex_autotune_replay.py) — Capture kernel tuning decisions from `max-autotune` and replay them deterministically. Useful for locking in performance-critical configurations.

### Grid Sweep

[`examples/flex_attention/flex_grid_sweep.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/flex_grid_sweep.py) — Exhaustive manual sweep over kernel configurations (block sizes, stages, warps) for a given attention pattern and problem size. Edit `MASK_MOD` and `SCORE_MOD` at the top of the file to test your specific pattern.

```bash
python examples/flex_attention/flex_grid_sweep.py --B 4 --H 32 --S 8192 --D 128
```

## Paged Attention

[`examples/flex_attention/paged_attention.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/paged_attention.py) — Efficient inference examples using FlexAttention with batches of variable-length KV caches. KV tensors are split into fixed-size pages and stored compactly instead of padding to the maximum length.
...
See [`paged_attention.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/paged_attention.py) for the full example implementation.

## ROWS_GUARANTEED_SAFE Demo

[`examples/flex_attention/rows_guaranteed_safe_demo.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/rows_guaranteed_safe_demo.py) — Demonstrates the subtle `ROWS_GUARANTEED_SAFE` contract in BlockMask metadata. Shows how block-level metadata can declare a block "safe" even when individual rows within it have no surviving attention entries, leading to NaN under `torch.compile` but not in eager mode.

```bash
python examples/flex_attention/rows_guaranteed_safe_demo.py
```
...
[`examples/flex_attention/aoti_create_block_mask.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/aoti_create_block_mask.py) — Export `create_block_mask` as an AOTInductor package with dynamic batch/head dimensions. Useful for deploying block mask creation outside of Python.

```bash
python examples/flex_attention/aoti_create_block_mask.py
```

### Full FlexAttention Export

[`examples/flex_attention/aoti_flex_attention.py`](https://github.com/meta-pytorch/attention-gym/blob/main/examples/flex_attention/aoti_flex_attention.py) — Export the full `flex_attention` forward (with LSE) as an AOTInductor artifact that can be serialized, saved, and reloaded.

```bash
python examples/flex_attention/aoti_flex_attention.py --mode aoti    # AOTInductor backend
python examples/flex_attention/aoti_flex_attention.py --mode python  # Python inductor backend
```
