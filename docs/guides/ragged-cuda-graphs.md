...
Putting the whole KDA variant of `DeltaRuleAttention` back together: QKV and output projections, normalization, short convolution, masking, the KDA core and backward. This Kimi-style shape uses {{ capacity_symbol("T_max") }} `=4096`, {{ capacity_symbol("N") }} `=32`, hidden size 2304 and 32 KDA heads.
