...
def test_max_guard_rejects_an_outlier_hidden_by_rms():
    high = torch.ones(1024, dtype=torch.float64)
    actual = high.clone()
    actual[0] += 0.1
    assert_rms_matches_low_precision_reference(actual, high, high, "outlier")
    with pytest.raises(AssertionError):
        assert_matches_low_precision_reference(actual, high, high, "outlier")


def test_rms_guard_rejects_widespread_error_hidden_by_max():
    high = torch.ones(1024, dtype=torch.float64)
    high[0] = 1024
    actual = high.clone()
    actual[1:] += 1
    assert_matches_low_precision_reference(actual, high, high, "widespread error")
    with pytest.raises(AssertionError):
        assert_rms_matches_low_precision_reference(actual, high, high, "widespread error")
