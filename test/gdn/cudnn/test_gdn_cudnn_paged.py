"""Paged prefill through the scalar-GDN cuDNN backend."""
...
def _expected_from_cudnn(inputs, initial_state, cu_seqlens):
...
def test_cudnn_paged_matches_gather_scatter() -> None:
    """Resumed, fresh, empty, and null routes match non-paged cuDNN bitwise."""
...
def test_cudnn_paged_dense_batch_without_mask_seeds_every_slot() -> None:
...
def test_cudnn_paged_rejects_grad_and_misaligned_slot_stride() -> None:
    q, k, value, gate, beta, _state, cu_seqlens = make_gdn_test_inputs(
...
def test_cudnn_paged_fullgraph_compile_and_cuda_graph_replay() -> None:
    q, k, value, gate, beta, _state, cu_seqlens = make_gdn_test_inputs(
