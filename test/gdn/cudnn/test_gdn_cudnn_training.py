"""Public autograd and framework integration for scalar-GDN cuDNN."""
...
_CUDNN_KERNEL_OPTIONS = {"backend": "cudnn"}


def chunk_gdn(*args, **kwargs):
    """Route fused calls in this module through the cuDNN backend."""
    if kwargs.get("impl") == "fused":
        kwargs.setdefault("kernel_options", _CUDNN_KERNEL_OPTIONS)
...
def test_public_gdn_cudnn_packed_forward_and_six_gradients() -> None:
...
def test_public_gdn_cudnn_grouped_h4_h12_forward_backward() -> None:
...
def test_public_gdn_cudnn_h64_custom_scale_forward_backward() -> None:
...
def test_public_gdn_cudnn_dense_batch_lowers_to_packed() -> None:
...
def test_gdn_cudnn_raw_operator_registration() -> None:
...
def test_gdn_cudnn_backward_fake_preserves_leading_strides() -> None:
...
"""
    ...
    assert result.returncode != 0, "invalid packed offsets reached the cuDNN kernel"
    assert "invalid packed cu_seqlens" in result.stderr, result.stdout + result.stderr


def test_public_gdn_cudnn_casts_low_precision_gate_and_beta_with_gradients() -> None:
...
def test_public_gdn_cudnn_fullgraph_forward_backward() -> None:
...
def test_public_gdn_cudnn_fullgraph_output_arities(mode: str) -> None:
...
def test_public_gdn_cudnn_dense_batch_fullgraph() -> None:
...
def test_public_gdn_cudnn_dynamic_packed_sequence_count() -> None:
...
def test_public_gdn_cudnn_dynamic_dense_tokens() -> None:
...
def test_public_gdn_cudnn_repeated_stateful_launches_cross_wave_boundary() -> None:
...
def test_public_gdn_cudnn_repeated_backward_crosses_wave_boundary() -> None:
...
def test_public_gdn_cudnn_cuda_graph_replay() -> None:
