"""Reject malformed cuDNN metadata before launching device-side validation on any CUDA GPU."""
...
from attn_gym.linear.gdn.impl import cudnn_ops
...
@pytest.mark.parametrize(
    "op",
    [
        cudnn_ops.chunk_gdn_cudnn_packed_fwd_op,
        cudnn_ops.chunk_gdn_cudnn_packed_fwd_with_initial_state_op,
        cudnn_ops.chunk_gdn_cudnn_packed_fwd_with_state_op,
    ],
...
def test_raw_forward_rejects_metadata_before_device_reads(op, invalid: str, monkeypatch):
    ...
    backend = Mock(side_effect=AssertionError("malformed metadata reached the cuDNN backend"))
    monkeypatch.setattr(chunk_scheduler, "_prepare_ragged_chunk_offsets", scheduler)
    monkeypatch.setattr(cudnn_ops, "_forward_backend", backend)
