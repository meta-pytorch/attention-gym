"""Layout, alignment, and address-width coverage for private GDN cuDNN launchers."""
...
def test_gdn_cudnn_outer_strides_match_compact_forward_and_backward() -> None:
...
def test_gdn_cudnn_forward_rejects_misaligned_and_noncontiguous_inner_modes() -> None:
...
def test_gdn_cudnn_active_offsets_past_int32_match_compact() -> None:
    ...
    expected_backward = chunk_gdn_bwd_cudnn_packed(q, k, value, gate, beta, d_output, cu_seqlens)
...
def test_gdn_cudnn_oversized_singleton_stride_executes_int64_path() -> None:
...
def test_gdn_cudnn_terminal_capacity_slack_is_not_read() -> None:
