...
from attn_gym.linear._delta_rule.cudnn.gdn_backward import chunk_gdn_bwd_cudnn_packed
from attn_gym.linear._delta_rule.cudnn.gdn_forward import run_forward
...
def assert_gradients_match_references(
    ...
    actual_gradients = chunk_gdn_bwd_cudnn_packed(
        *inputs[:5],
        d_output,
        inputs[6],
        initial_state,
...
def test_gdn_cudnn_backward_matches_curated_matrix(
    lengths: tuple[int, ...],
    dtype: torch.dtype,
    key_heads: int,
...
def test_gdn_cudnn_backward_uniform_negative_twenty_is_finite(
    ...
    gradients = chunk_gdn_bwd_cudnn_packed(*inputs[:5], d_output, inputs[6])[:5]
...
def test_gdn_cudnn_backward_checkpoint_uses_v_by_k_state_at_token_64() -> None:
    ...
    actual_gradients = chunk_gdn_bwd_cudnn_packed(q, k, value, gate, beta, d_output, cu_seqlens)
...
def test_gdn_cudnn_packed_differential_fuzz(
