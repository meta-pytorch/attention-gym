...
CONV_PLANS = [
    ContextParallelPlan.from_fragments(CONV_CU_SEQLENS, CONV_FRAGMENTS, rank) for rank in range(4)
]
...
def test_plan_rejects_ranges_that_do_not_tile_the_stream(cu_seqlens, fragments, message):
...
def test_plan_handles_leading_and_trailing_empty_sequences():
    ...
    with pytest.raises(ValueError, match="empty"):
        ContextParallelPlan.from_fragments((0, 0), [[(0, 0)]], cp_rank=0)
...
def test_exit_cotangents_fold_successors_in_reverse_and_add_the_direct_term():
    ...
    zero = torch.zeros(1, VALUE_DIM, KEY_DIM)
    # Subsequence 1 (sequence 1) continues on ranks 1 then 2; the fold visits the farthest first.
