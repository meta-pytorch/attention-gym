...
from attn_gym.linear import ReplayState, chunk_kda, paged_chunk_kda
...
def test_paged_chunk_kda_updates_strided_pool(state_dtype: torch.dtype):
    ...
    storage, pool = strided_state_pool(6, q.shape[2], 128, 128, prefix=0, dtype=state_dtype)
    ...
    expected_output, expected_state = chunk_kda(
        ...
        cumulative_gate,
        beta,
        expected_pool[slots.long()].float(),
        cu_seqlens=cu_seqlens,
        output_final_state=True,
    )
    assert expected_state is not None
    expected_pool[slots.long()] = expected_state.to(state_dtype)
...
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_paged_chunk_kda_matches_pytorch(state_dtype: torch.dtype):
    ...
    state_cache = torch.randn(4, 1, 128, 128, device="cuda", dtype=state_dtype)
    expected_cache = state_cache.clone()
    initial_state = expected_cache[state_indices.long()].float()
...
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_chunk_kda_paged_op_registration(state_dtype: torch.dtype):
    ...
    _storage, pool = strided_state_pool(5, q.shape[2], 128, 128, prefix=0, dtype=state_dtype)
    slots = torch.tensor([4, 2], device="cuda", dtype=torch.int32)
    has_initial_state = torch.tensor([True, False], device="cuda")
...
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_paged_chunk_kda_fullgraph_compile(state_dtype: torch.dtype):
    ...
    eager_storage, eager_pool = strided_state_pool(
        6, inputs[0].shape[2], 128, 128, prefix=0, dtype=state_dtype
    )
    compiled_storage, compiled_pool = strided_state_pool(
        6, inputs[0].shape[2], 128, 128, prefix=0, dtype=state_dtype
    )
...
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_paged_chunk_kda_cuda_graph_replay(state_dtype: torch.dtype):
    ...
    cu_seqlens = torch.tensor([0, 64, 192], device="cuda", dtype=torch.int32)
    slots = torch.tensor([5, 2], device="cuda", dtype=torch.int32)
    storage, pool = strided_state_pool(7, q.shape[2], 128, 128, prefix=0, dtype=state_dtype)
    paged_chunk_kda(
        q,
        k,
        v,
...
@pytest.mark.parametrize(
    "num_sequences,tokens,heads,active",
    [
        pytest.param(1, 4096, 32, False, id="large-tile-dimension"),
        pytest.param(256, 32, 2, True, id="many-short-sequences"),
    ],
)
def test_paged_chunk_kda_replay_prefill_scaling(
    num_sequences: int,
    tokens: int,
    heads: int,
    active: bool,
):
    """Replay prefill supports large feature grids and many short requests."""
    if active:
        inputs = tuple(
            tensor.detach() for tensor in _inputs(batch=num_sequences, tokens=tokens, heads=heads)
        )
        expected, _ = chunk_kda(
            *inputs,
            autotune=False,
        ...
        state_indices = torch.arange(1, num_sequences + 1, device="cuda", dtype=torch.int32)
        num_slots = num_sequences + 1
    else:
        shape = (num_sequences, tokens, heads, 128)
        inputs = (
            torch.zeros(shape, device="cuda", dtype=torch.bfloat16),
            torch.zeros(shape, device="cuda", dtype=torch.bfloat16),
            torch.zeros(shape, device="cuda", dtype=torch.bfloat16),
            torch.zeros(shape, device="cuda"),
            torch.zeros(shape[:-1], device="cuda"),
        )
        expected = torch.zeros_like(inputs[2])
        state_indices = torch.zeros(num_sequences, device="cuda", dtype=torch.int32)
        num_slots = 1

    packed_inputs = tuple(
        tensor.view(1, num_sequences * tokens, *tensor.shape[2:]) for tensor in inputs
    )
    state_cache = torch.zeros((num_slots, heads, 128, 128), device="cuda")
    replay_state = ReplayState(
        *(tensor.new_zeros((num_slots, 64, *tensor.shape[2:])) for tensor in packed_inputs),
        torch.zeros(num_slots, device="cuda", dtype=torch.int32),
    )
    cu_seqlens = torch.arange(num_sequences + 1, device="cuda", dtype=torch.int32) * tokens
    ...
    output = paged_chunk_kda(
        *packed_inputs,
        ...
        cu_seqlens=cu_seqlens,
        autotune=False,
        kernel_options={"schedule": "persistent"},
        replay_state=replay_state,
    )

    assert torch.equal(output.view_as(expected), expected)
    expected_count = tokens % 64 if active else 0
    assert torch.equal(
        replay_state.count[state_indices[state_indices > 0].long()],
        replay_state.count.new_full((num_sequences if active else 0,), expected_count),
    )
