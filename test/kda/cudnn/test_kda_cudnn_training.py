...
def _candidate_dense(*inputs: torch.Tensor) -> torch.Tensor:
    ...
    output, _ = chunk_kda(*inputs[:5], kernel_options={"backend": "cudnn"})
    return output
...
def _candidate_dense_split(*inputs: torch.Tensor) -> torch.Tensor:
    ...
    output, _ = chunk_kda(
        *inputs[:5],
        kernel_options={"backend": "cudnn", "split_backward": True},
    )
    return output


@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cudnn_training_fullgraph_and_six_gradients(dtype: torch.dtype) -> None:
...
def test_cudnn_outer_strided_initial_state_fullgraph_gradients(
    state_layout,
    cotangent_layout,
) -> None:
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cudnn_initial_state_without_final_state_fullgraph_gradients(dtype: torch.dtype) -> None:
...
def test_cudnn_initial_state_without_final_state_selects_output_only_op(monkeypatch) -> None:
    from attn_gym.linear import chunk_kda
    ...
    output_only_op = backend.chunk_cudnn_packed_fwd_with_initial_state_op
    ...
    monkeypatch.setattr(
        backend, "chunk_cudnn_packed_fwd_with_initial_state_op", record_output_only
    )
    monkeypatch.setattr(
        backend,
...
def test_cudnn_custom_scale_matches_fused_forward_and_gradients() -> None:
    from attn_gym.linear import chunk_kda
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cudnn_dense_training_fullgraph_and_gradients(dtype: torch.dtype) -> None:
...
def test_cudnn_saturated_gate_six_gradients_are_finite() -> None:
...
def test_cudnn_local_backward_split_matches_exact_gradients() -> None:
    from attn_gym.linear.kda.impl.cudnn_ops import chunk_cudnn_packed_local_bwd_op
    ...
    for mode in ("contracting", "no_forgetting"):
        ...
        high_grads = _reference_gradients(high_precision, high_targets, (d_output,))
        low_grads = _reference_gradients(low_precision, low_targets, (d_output,))
        actual = chunk_cudnn_packed_local_bwd_op(
            q.detach(),
            k.detach(),
            value.detach(),
            gate.detach(),
...
def test_cudnn_packed_local_backward_matches_exact_gradients(
...
def test_cudnn_public_packed_unsplit_local_backward_matches_exact_gradients(monkeypatch) -> None:
    ...
    selected = []
    local_backward_op = backend.chunk_cudnn_packed_local_bwd_op

    def record_local_backward(*args):
        selected.append(args[-2])
        return local_backward_op(*args)

    monkeypatch.setattr(backend, "chunk_cudnn_packed_local_bwd_op", record_local_backward)
...
def test_cudnn_split_schedules_reject_stateful_calls(option: str) -> None:
    from attn_gym.linear import chunk_kda
...
def _split_forward_pair(
    ...
    """Unsplit and forgetting-horizon cuDNN forwards of one long dense stream.
...
def test_cudnn_cpu_inputs_fail_before_dispatch() -> None:
    from attn_gym.linear import chunk_kda
    ...
    with pytest.raises(ValueError, match="cuDNN KDA backend requires CUDA tensors"):
        chunk_kda(q, k, value, gate, beta, kernel_options={"backend": "cudnn"})
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cudnn_local_backward_fullgraph(dtype: torch.dtype) -> None:
    expected_inputs = list(_make_inputs(requires_grad=True, dtype=dtype))
    actual_inputs = list(_make_inputs(requires_grad=True, dtype=dtype))
    dense_cu = torch.tensor([0, expected_inputs[0].shape[1]], dtype=torch.int32, device="cuda")
    expected_inputs[-1] = dense_cu
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cudnn_composed_state_only_backward(dtype: torch.dtype) -> None:
    actual_inputs = _make_inputs(requires_grad=True, dtype=dtype)
    high_precision, low_precision, high_targets, low_targets = _references(actual_inputs)
    actual = torch.compile(_candidate, fullgraph=True)(*actual_inputs)
...
def test_cudnn_dense_tail_takes_the_packed_path() -> None:
    ...
    output, _ = chunk_kda(*implicit, kernel_options={"backend": "cudnn"})
    expected, _ = chunk_kda(*explicit, cu_seqlens=offsets, kernel_options={"backend": "cudnn"})
...
def test_cudnn_rejects_mismatched_value_and_beta_shapes() -> None:
    from attn_gym.linear import chunk_kda
...
def test_cudnn_packed_tail_isolated_from_next_sequence() -> None:
    from attn_gym.linear import chunk_kda
...
def test_cudnn_forward_op_registration(dtype: torch.dtype, outer_strided: bool) -> None:
    ...
    local_gradients = chunk_cudnn_packed_local_bwd_op(
...
def test_cudnn_initial_state_forward_op_fullgraph() -> None:
    from attn_gym.linear.kda.impl.cudnn_ops import chunk_cudnn_packed_fwd_with_initial_state_op

    inputs = _make_inputs(requires_grad=False)
    args = (*inputs, SCALE)
    expected = chunk_cudnn_packed_fwd_with_initial_state_op(*args)
...
def test_cudnn_no_state_fullgraph_cuda_graph_replay(candidate) -> None:
    inputs = _make_inputs(requires_grad=False)
...
def test_cudnn_tma_validation_routes_oversized_singleton_stride_to_int64() -> None:
