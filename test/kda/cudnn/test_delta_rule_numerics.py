...
pytest.importorskip(
    ...
    reason="the cuDNN numerical tests require nvidia-cutlass-dsl>=4.7",
)
...
pytestmark = pytest.mark.skipif(
    ...
    reason="the cuDNN numerical tests require SM100 or SM103",
)
...
def _gdn_result(
    ...
    *,
    ...
    """Run GDN through cuDNN or its eager recurrence and differentiate the output."""
    leaves = clone_kda_inputs(inputs, dtype=precision)
    if precision is None:
        output = chunk_gdn(*leaves, impl="fused", kernel_options=_CUDNN)[0]
    else:
...
def test_cudnn_exact_orthogonal_writes(family: str) -> None:
...
def test_kda_cudnn_delta_residual_keeps_fp32_precision(dtype: torch.dtype) -> None:
    ...
    output = chunk_kda(*inputs, scale=1.0, autotune=False, kernel_options=_CUDNN)[0]
    cu_seqlens = torch.tensor([0, tokens], device="cuda", dtype=torch.int32)
    dq, _, _, _, dbeta = chunk_cudnn_packed_local_bwd_op(*inputs, d_output, cu_seqlens, False, 1.0)
...
@pytest.mark.parametrize("seed", (888, 889, 890))
def test_kda_cudnn_forward_and_public_backward_model_range(seed: int) -> None:
    """cuDNN forward and the public fallback backward must meet the BF16 error budget."""
    inputs = make_kda_test_inputs(128, heads=2, seed=seed, requires_grad=True)
    d_output = _d_output_like(inputs[2], seed + 1)
    actual_inputs = clone_kda_inputs(inputs)
    output = chunk_kda(*actual_inputs, output_final_state=False, kernel_options=_CUDNN)[0]
...
@pytest.mark.parametrize("seed", (888, 889, 890))
def test_kda_cudnn_raw_backward_precision(seed: int) -> None:
    """Raw cuDNN backward must stay within the BF16 reference error budget."""
    inputs = make_kda_test_inputs(128, heads=2, seed=seed, requires_grad=True)
    d_output = _d_output_like(inputs[2], seed + 1)
