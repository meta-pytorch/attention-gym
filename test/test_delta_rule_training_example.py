...
# Aggregate budgets (bf16 eps units) for same-kernel comparisons that differ only in
# partitioning; calibrated on GB200.
PACKED_RMS_EPS = 4.0
MASKED_RMS_EPS = 4.0
...
def test_example_writes_fastmath_and_kernel_options_through_to_the_core(monkeypatch, variant):
    """The module owns no kernel policy: both knobs reach the gate and chunk core unchanged."""
    ...
    monkeypatch.setattr(
        delta_rule_training, "gate_transform", record("gate", delta_rule_training.gate_transform)
    )
    monkeypatch.setattr(
        delta_rule_training, core_name, record("core", getattr(delta_rule_training, core_name))
    )
    ...
    model(torch.randn(1, 64, 32, device="cuda"))
    assert seen["gate"]["fastmath"] is True and seen["gate"]["impl"] == "fused"
    assert seen["core"]["kernel_options"] is options
    if variant == "kda":
...
@pytest.mark.parametrize("variant", ["kda", "gdn"])
def test_example_reference_backend_accepts_low_precision_compute(variant):
    """Reference Q/K normalization promotes to FP32; V must follow so chunk_* sees one dtype."""
    torch.manual_seed(6)
    options = {"hidden_size": 32, "num_heads": 1, "head_dim": 128, "variant": variant}
    fp32 = DeltaRuleAttention(**options, backend="reference", device="cuda")
    bf16 = DeltaRuleAttention(
        **options, backend="reference", compute_dtype=torch.bfloat16, device="cuda"
    )
    bf16.load_state_dict(fp32.state_dict())
    hidden = torch.randn(1, 64, 32, device="cuda")
    expected = fp32(hidden, return_final_state=True)
    actual = bf16(hidden, return_final_state=True)
    for name, value, target in (
        ("output", actual.hidden_states, expected.hidden_states),
        ("final_state", actual.final_state, expected.final_state),
    ):
        torch.testing.assert_close(
            value.float(), target, rtol=0, atol=5e-2 * target.abs().max().item(), msg=name
        )
        assert_relative_rms_within(value, target, name, max_eps=8)
...
def test_example_packed_matches_sequence_for_loop(monkeypatch, compute_dtype, variant):
    ...
    gate = delta_rule_training.gate_transform
    ...
    monkeypatch.setattr(delta_rule_training, "gate_transform", record_gate)
    ...
    assert_relative_rms_within(actual, expected, "output", max_eps=PACKED_RMS_EPS)
    ...
    assert_relative_rms_within(
        actual_gradient, expected_gradient, "input gradient", max_eps=PACKED_RMS_EPS
    )
...
def _assert_masked_run_matches(names, actual, expected, active_tokens):
    ...
    for label, value, target in zip(("output", "input gradient"), actual[:2], expected[:2]):
        active = value[:, :active_tokens]
        torch.testing.assert_close(active, target, rtol=3e-2, atol=3e-2, msg=label)
        assert_relative_rms_within(active, target, label, max_eps=MASKED_RMS_EPS)
    ...
    for name, value, target in zip(names, actual[2:], expected[2:], strict=True):
        ...
        assert_relative_rms_within(value, target, name, max_eps=MASKED_RMS_EPS)
