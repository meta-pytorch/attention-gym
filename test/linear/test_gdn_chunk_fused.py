...
@pytest.mark.parametrize("head_dim", [64, 128])
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_paged_chunk_matches_gather_scatter(state_dtype: torch.dtype, head_dim: int):
...
@pytest.mark.parametrize("head_dim", [64, 128])
@pytest.mark.parametrize("state_dtype", [torch.float32, torch.bfloat16])
def test_paged_chunk_cuda_graph_replay(state_dtype: torch.dtype, head_dim: int):
