...
def test_naive_chunk_gradients_match_recurrent():
    ...
    gradients = []
    for implementation in ("recurrent", "chunk", "cumulative"):
        differentiable_inputs = [value.clone().requires_grad_() for value in inputs]
        q, k, v, g, beta, initial_state = differentiable_inputs
    ...
    for actual_gradients in gradients[1:]:
        for actual, recurrent in zip(actual_gradients, gradients[0]):
