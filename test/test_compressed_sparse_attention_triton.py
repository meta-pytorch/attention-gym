...
PATHOLOGICAL_SHAPES = [
    ...
    pytest.param(
        True,
        {"heads": 3, "sequence_length": 17, "topk": 0, "window": 9},
        id="local-only",
    ),
    pytest.param(
        True,
        {"heads": 3, "sequence_length": 17, "topk": 3, "window": 0},
        id="compressed-only",
    ),
]

SPARSE_BACKWARD_SHAPES = [
    pytest.param(
        {
            "sequence_length": 65,
            "compression_rate": 16,
        ...
        id="compressed-only-repeated-selection",
    ),
    pytest.param(
        {
            "sequence_length": 35,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required")
@pytest.mark.parametrize("overrides", SPARSE_BACKWARD_SHAPES)
def test_triton_sparse_backward_edge_cases_match_reference(overrides):
    base = make_inputs(
        False,
        torch.float32,
        batch=1,
